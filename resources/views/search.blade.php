...
<div class="max-w-[1280px] mx-auto px-4 md:px-8 py-6 md:py-8">
    ...
    <div class="mb-10 w-full max-w-3xl mx-auto">
        ...
        <form action="{{ route('search') }}" method="GET" class="relative group">
            <input name="q" value="{{ $q ?? '' }}" class="w-full pl-6 pr-6 py-4 bg-white border border-gray-200 hover:border-gray-300 focus:border-primary focus:ring-1 focus:ring-primary rounded-xl text-lg transition-all shadow-sm outline-none text-[#333]" placeholder="Search by title, author, or genre..." type="text" />
            <button type="submit" class="absolute right-3 top-1/2 -translate-y-1/2 bg-primary text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-primary/90 transition-colors">Search</button>
        </form>
    </div>
